...
class BasicBlock3D(BaseModule):
    def __init__(self, channels_in, channels_out, stride=1, downsample=None, init_cfg=None):
        super(BasicBlock3D, self).__init__(init_cfg=init_cfg)
...
class CustomResNet3D(BaseModule):
    def __init__(
        ...
        super(CustomResNet3D, self).__init__(init_cfg=init_cfg)
        ...
        for i in range(len(num_layer)):
            ...
            curr_numC = num_channels[i]
            layer.extend(
                [BasicBlock3D(curr_numC, curr_numC) for _ in range(num_layer[i] - 1)]
            )
            layers.append(nn.Sequential(*layer))
