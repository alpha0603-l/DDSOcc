...
try:
    from mmdet3d.models.losses.lovasz_loss import LovaszLoss
except ImportError:
    print("Warning: LovaszLoss not found in mmdet3d.models.losses.lovasz_loss")
    LovaszLoss = None
...
class BEVOCCHead2D(BaseModule):
    def __init__(self,
        ...
        if LovaszLoss is not None:
            self.loss_lovasz_fn = LovaszLoss(loss_weight=1.0, ignore_index=255)
        else:
            self.loss_lovasz_fn = None
    ...
    def loss(self, occ_pred, voxel_semantics, mask_camera):
        ...
        if self.use_mask:
            ...
            loss_occ_ce = self.loss_occ(
                preds,  # (N, n_cls)
                voxel_semantics,  # (N, )
                mask_camera,  # weight
                avg_factor=num_total_samples
            )
            loss['loss_occ'] = loss_occ_ce
            if self.loss_lovasz_fn is not None:
                valid_mask = mask_camera.bool()
                if valid_mask.sum() > 0:
                    preds_valid = preds[valid_mask].softmax(dim=-1)  # (N_valid, n_cls)
                    targets_valid = voxel_semantics[valid_mask]  # (N_valid, )
                    loss_lovasz = self.loss_lovasz_fn(preds_valid, targets_valid)
                    loss['loss_lovasz'] = 2.0 * loss_lovasz
            ...
            loss['loss_occ'] = 10.0 * loss_occ_ce
            if self.loss_lovasz_fn is not None:
                loss_lovasz = self.loss_lovasz_fn(preds.softmax(dim=-1), voxel_semantics)
                loss['loss_lovasz'] = 1.0 * loss_lovasz
