import torch
try:
    from mmdet.models.task_modules.samplers import RandomSampler, SamplingResult
except ImportError:
...
class IoUNegPiecewiseSampler(RandomSampler):
        ...
        if len(neg_inds) <= num_expected:
            ...
            for piece_inds in range(self.neg_piece_num):
                if piece_inds == self.neg_piece_num - 1:  # for the last piece
                    ...
                    piece_expected_num = (
                        int(num_expected * self.neg_piece_fractions[piece_inds]) + extend_num
                    )
                    min_iou_thr = self.neg_iou_thr[piece_inds + 1]
                max_iou_thr = self.neg_iou_thr[piece_inds]
                piece_neg_inds = torch.nonzero(
                    (max_overlaps >= min_iou_thr) & (max_overlaps < max_iou_thr), as_tuple=False
                ).view(-1)
    ...
    def sample(self, assign_result, bboxes, gt_bboxes, gt_labels=None, **kwargs):
        ...
        pos_inds = self._sample_pos( # [修复] 直接调用 self._sample_pos 而不是 self.pos_sampler
        ...
        neg_inds = self._sample_neg( # [修复] 直接调用 self._sample_neg 而不是 self.neg_sampler
