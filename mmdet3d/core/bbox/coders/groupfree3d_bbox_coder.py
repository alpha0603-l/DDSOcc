...
class GroupFree3DBBoxCoder(PartialBinBasedBBoxCoder):
    def __init__(self, num_dir_bins, num_sizes, mean_sizes, with_rot=True, size_cls_agnostic=True):
        super(GroupFree3DBBoxCoder, self).__init__(
            num_dir_bins=num_dir_bins, num_sizes=num_sizes, mean_sizes=mean_sizes, with_rot=with_rot
        )
    ...
    def encode(self, gt_bboxes_3d, gt_labels_3d):
        ...
        size_res_target = (
            gt_bboxes_3d.dims - gt_bboxes_3d.tensor.new_tensor(self.mean_sizes)[size_class_target]
        )
    ...
    def decode(self, bbox_out, prefix=""):
        ...
        if self.size_cls_agnostic:
            ...
            size_res = torch.gather(
                bbox_out[f"{prefix}size_res"], 2, size_class.unsqueeze(-1).repeat(1, 1, 1, 3)
            )
