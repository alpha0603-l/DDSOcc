...
dvr_source_dir = os.path.join(project_root, 'lib', 'dvr')

sources = [
    os.path.join(dvr_source_dir, "dvr.cpp"),
    os.path.join(dvr_source_dir, "dvr.cu")
]
...
def calc_metrics(pcd_pred_list, pcd_gt_list):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        for j, threshold in enumerate(thresholds):
            denominator = gt_cnt + pred_cnt - tp_cnt[j]
            iou_val = np.divide(tp_cnt[j], denominator, out=np.zeros_like(tp_cnt[j]), where=denominator != 0)
            iou_list.append(iou_val[:-1])

    return iou_list


def calc_ray_pq(sem_pred_list, sem_gt_list, inst_pred_list, inst_gt_list, lidar_origin_list):
    ...
    for sem_pred, sem_gt, inst_pred, inst_gt, lidar_origins in \
        ...
        if pcd_pred.shape[0] == 0: continue
...
def calc_ray_iou(sem_pred_list, sem_gt_list, lidar_origin_list):
    torch.cuda.empty_cache()
    ...
    if len(pcd_pred_list) == 0:
        print("Warning: No valid samples for Ray IoU calculation!")
        return {}
