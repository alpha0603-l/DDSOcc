...
class Metric_RayPQ:
    def __init__(self,
        ...
        self.pan_iou = np.zeros([len(self.thresholds), num_classes], dtype=np.float64)
    ...
    def count_pq(self):
        sq_all = self.pan_iou.astype(np.float64) / np.maximum(
            self.pan_tp.astype(np.float64), self.eps)
        rq_all = self.pan_tp.astype(np.float64) / np.maximum(
            self.pan_tp.astype(np.float64) + 0.5 * self.pan_fp.astype(np.float64)
            + 0.5 * self.pan_fn.astype(np.float64), self.eps)
