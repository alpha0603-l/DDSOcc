...
class GroupingOperation(Function):
    ...
    """
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)  # 强制 float32，避免半精度下 CUDA 核出错
    def forward(ctx, features: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
        ...
        """
        if group_points_ext is None:
            raise ImportError("group_points_ext has not been compiled.")
        ...
        _, C, N = features.size()
        output = torch.zeros(
            (B, C, nfeatures, nsample),
            dtype=features.dtype,
            device=features.device
        )
        group_points_ext.group_points_wrapper(
            B, C, N, nfeatures, nsample, features, indices, output
        )
    ...
    def backward(ctx, grad_out: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        group_points_ext.group_points_grad_wrapper(
            B, C, N, npoint, nsample, grad_out_data, idx, grad_features
        )
            ...
            for i_batch in range(idx.shape[0]):
                for i_region in range(idx.shape[1]):
                    ...
                    sample_ind = torch.randint(
                        0, num_unique, (self.sample_num - num_unique,), dtype=torch.long, device=idx.device
                    )
