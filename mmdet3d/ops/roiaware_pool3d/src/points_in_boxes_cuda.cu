...
__global__ void points_in_boxes_aux_batch_kernel(int batch_size, int boxes_num,
                                             int pts_num, const float *boxes,
                                             const float *pts,
                                             int *box_idx_of_points, float *box_center_reg_of_points) {
  // params boxes: (B, N, 7) [x, y, z, w, l, h, rz] in LiDAR coordinate, z is
  // the bottom center, each box DO NOT overlaps params pts: (B, npoints, 3) [x,
  // y, z] in LiDAR coordinate params boxes_idx_of_points: (B, npoints), default
  // -1
  ...
  pts += bs_idx * pts_num * 3 + pt_idx * 3;
  box_idx_of_points += bs_idx * pts_num * boxes_num + pt_idx * boxes_num;
  box_center_reg_of_points += bs_idx * pts_num * 3 + pt_idx * 3;
  ...
  for (int k = 0; k < boxes_num; k++) {
    ...
    if (cur_in_flag) {
      box_idx_of_points[k] = 1;
      box_center_reg_of_points[0] = pts[0] -  (boxes + k * 7)[0];
      box_center_reg_of_points[1] = pts[1] -  (boxes + k * 7)[1];
      box_center_reg_of_points[2] = pts[2] -  (boxes + k * 7)[2];
    }
    cur_in_flag = 0;
  }
}

void points_in_boxes_aux_batch_launcher(int batch_size, int boxes_num, int pts_num,
                                    const float *boxes, const float *pts,
                                    int *box_idx_of_points, float *box_center_reg_of_points) {
  // params boxes: (B, N, 7) [x, y, z, w, l, h, rz] in LiDAR coordinate, z is
  // the bottom center, each box params pts: (B, npoints, 3) [x, y, z] in
  ...
  dim3 blocks(DIVUP(pts_num, THREADS_PER_BLOCK), batch_size);
  dim3 threads(THREADS_PER_BLOCK);
  points_in_boxes_aux_batch_kernel<<<blocks, threads>>>(
      batch_size, boxes_num, pts_num, boxes, pts, box_idx_of_points, box_center_reg_of_points);
...
int points_in_boxes_aux_batch(at::Tensor boxes_tensor, at::Tensor pts_tensor,
                          at::Tensor box_idx_of_points_tensor, at::Tensor box_center_reg_of_points_tensor) {
  // params boxes: (B, N, 7) [x, y, z, w, l, h, rz] in LiDAR coordinate, z is
  // the bottom center. params pts: (B, npoints, 3) [x, y, z] in LiDAR
  // coordinate params boxes_idx_of_points: (B, npoints), default -1
  ...
  CHECK_INPUT(pts_tensor);
  CHECK_INPUT(box_idx_of_points_tensor);
  CHECK_INPUT(box_center_reg_of_points_tensor);
  ...
  const float *pts = pts_tensor.data_ptr<float>();
  int *box_idx_of_points = box_idx_of_points_tensor.data_ptr<int>();
  float *box_center_reg_of_points = box_center_reg_of_points_tensor.data_ptr<float>();

  points_in_boxes_aux_batch_launcher(batch_size, boxes_num, pts_num, boxes, pts,
                                 box_idx_of_points, box_center_reg_of_points);
