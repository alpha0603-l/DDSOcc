...
int points_in_boxes_aux_cpu(at::Tensor boxes_tensor, at::Tensor pts_tensor,
                            at::Tensor pts_indices_tensor, at::Tensor pts_reg_tensor) {
  // params boxes: (N, 7) [x, y, z, w, l, h, rz] in LiDAR coordinate, z is the
  // bottom center, each box DO NOT overlaps params pts: (npoints, 3) [x, y, z]
  // in LiDAR coordinate params pts_indices: (N, npoints)
  ...
  const float *pts = pts_tensor.data_ptr<float>();
  int *pts_indices = pts_indices_tensor.data_ptr<int>();
  float *pts_reg = pts_reg_tensor.data_ptr<float>();
  ...
  for (int i = 0; i < boxes_num; i++) {
    for (int j = 0; j < pts_num; j++) {
      ...
      if (cur_in_flag == 1)
      {
        pts_reg[j*3] = pts[j*3] - boxes[i*7];
        pts_reg[j*3+1] = pts[j*3+1] - boxes[i*7+1];
        pts_reg[j*3+2] = pts[j*3+2] - (boxes[i*7+2] + boxes[i*7+5] / 2.0);
      }
    }
  }
