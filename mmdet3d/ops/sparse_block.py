...
class SparseBottleneck(Bottleneck, spconv.SparseModule):
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        # [Fix] spconv 2.x: use replace_feature
        # [Fix] mmdet naming: bn1 -> norm1
        ...
        if self.downsample is not None:
            # [Fix] extract features from sparse tensor
            identity_obj = self.downsample(x)
            identity = identity_obj.features
