        ...
        if metric == 'Ray-IoU':
            ...
            if self.data_type == 'occ3d':
                ...
                for index, occ_pred in enumerate(tqdm(occ_results)):
                    ...
                    if show_dir is not None:
                        ...
                        try:
                            ...
                            import mmengine
                            mmengine.mkdir_or_exist(show_dir)

                        # scene_name = info['scene_name']
                        scene_name = [tem for tem in info[self.data_type]['occ_path'].split('/') if 'scene-' in tem][0]
                        sample_token = info['token']

                        scene_path = os.path.join(show_dir, scene_name, sample_token)
                        try:
                            mmcv.mkdir_or_exist(scene_path)
                        except AttributeError:
                            import mmengine
                            mmengine.mkdir_or_exist(scene_path)

                        save_path = os.path.join(scene_path, 'pred.npz')
                ...
                for index, occ_pred in enumerate(tqdm(occ_results)):
                    ...
                    SSC_metric, SSC_occ_metric = openocc_metric(occ_pred, gt, eval_type='SSC', empty_idx=0,
                                                                visible_mask=None)
